...
고객용 MCP 런타임은 `bin/quilla-mcp.exe`입니다. 비공개 TypeScript server
source와 빌드된 `server/lib` 출력물은 개발용 reference이므로 공개 릴리스
패키지에 포함되지 않는 것이 정상입니다.
...
`v0.2.0` 기준 manifest는 다음 값을 보고해야 합니다.
...
Quilla `0.2.0` Windows binary는 code-signed 상태가 아닙니다. 현재 릴리스에서는
이 상태가 정상입니다. Package manifest, GitHub release provenance,
SHA-256 hash를 신뢰 확인 장치로 사용하세요.
